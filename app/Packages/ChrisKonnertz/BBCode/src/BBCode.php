...
namespace App\Packages\ChrisKonnertz\BBCode;
...
class BBCode
{
    ...
    protected function generateTag(Tag $tag, &$html, Tag $openingTag = null, array $openTags = [])
    {
        ...
        if (isset($openTags['code']) && sizeof($openTags['code']) > 0) {
            if ($tag->opening) {
                return (string)$tag;
            } else {
                if ($tag->name === 'code') {
                    // Support nested code tags.
                    foreach ($openTags['code'] as $openTag) {
                        if ($openTag->name === 'code') {
                            return (string)$tag;
                        }
                    }
                } else {
